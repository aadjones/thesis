...
Now consider the typical equation of motion, 
\begin{equation}
\MM \ddot{\xx} + \DD \dot{\xx} + \KK \xx = \ff
\end{equation}

We perform the substitution $\xx = \UU \qq$ for the appropriate choice $\qq \in S$, obtaining the following:

\begin{equation}
\MM {\UU \ddot{\qq}} + \DD {\UU \dot{\qq}} + \KK \UU \qq = \ff
\end{equation}

Next, we pre-multiply both sides by $\UU^{T}$:

\begin{equation}
\UU^{T} \MM \UU \ddot{\qq} + \UU^{T} \DD \UU \dot{\qq} + \UU^{T} \KK \UU \qq = \UU^{T} \ff
\end{equation}

By the mass-orthogonality condition, the term $\UU^{T} \MM \UU \ddot{\qq}$ reduces to $\II_r \ddot{\qq} = \ddot{\qq}$. 