...
Unlike the classic Chladni pattern case, CFD simulations do not yield a single matrix $\boldA$ on which an eigenvalue analysis can be performed. The non-linear operations involved generate a new, time-dependent matrix $\boldA$ at every timestep. Instead of one canonical $\boldA$ that can be said to characterize the behavior of the entire system, there are instead infinitely many $\boldA$s, none of which inherently take precedence over the others, as discussed in \S \ref{sec:general-subspace}. Fortunately, if some sort of precedence is imposed, a process akin to an eigenvalue analysis can still be applied.
